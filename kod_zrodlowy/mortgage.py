...
#Zbiór danych: Mortgage
mortgage = pd.read_csv('dane/mortgage.csv')

print(mortgage.head())
print(mortgage.info())

print("\nCzy w danych występują wartości NA lub null?")
print(mortgage.isna().any().any())
print(mortgage.isnull().any().any())

#Usunięcie nieistotnych dla badania zmiennych
mortgage = mortgage.drop('payoff_time', axis=1)
mortgage = mortgage.drop('status_time', axis=1)
mortgage = mortgage.drop('id', axis=1)
print(mortgage.head())
print(mortgage.info())

print("\nStopień niezbalansowania zbioru danych")
print(sum(mortgage['default_time'])/len(mortgage['default_time']))


#WSTĘPNA ANALIZA DANYCH

#Podstawowowe statystyki
print((mortgage.iloc[:, 0:5]).describe())
print((mortgage.iloc[:, 5:9]).describe())
print((mortgage.iloc[:, 10:14]).describe())
print((mortgage.iloc[:, 15:19]).describe())

#Współczynniki zmienności
for col_name in mortgage.columns:
    mean_mortgage = np.mean(mortgage[col_name])
    std_mortgage = np.std(mortgage[col_name])

    cv = (mean_mortgage/std_mortgage) * 100
    print("WSP Zmiennosci:\n" + col_name + ": ", cv)

#Zbadanie zależności między zmiennymi

"""correlation_matrix = mortgage.corr()
sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm')
plt.show()"""

#Wykresy pudełkowe w relacji do zmiennej default_time

"""for col_name in mortgage.columns:
    plt.figure(figsize=(8, 6))
    sns.boxplot(x='default_time', y=col_name, data=mortgage)
    plt.xlabel('default_time')
    plt.ylabel(col_name)
    plt.show()"""

#Usunięcie zmiennych nie wykazujących zależności
mortgage = mortgage.drop('balance_time', axis=1)
mortgage = mortgage.drop('REtype_CO_orig_time', axis=1)
mortgage = mortgage.drop('REtype_PU_orig_time', axis=1)
mortgage = mortgage.drop('REtype_SF_orig_time', axis=1)
mortgage = mortgage.drop('investor_orig_time', axis=1)
mortgage = mortgage.drop('balance_orig_time', axis=1)

print(mortgage.head())
print(mortgage.info())

mortgage['LTV_time'] = mortgage['LTV_time'].interpolate(method='linear')
print(mortgage.info())
...
axs = axs.flatten()
for i, column in enumerate(mortgage.columns[9:14]):
    if i < len(axs):
        axs[i].hist(mortgage[column])
        axs[i].set_title(column)
        axs[i].set_xlabel('Values')
        axs[i].set_ylabel('Frequency')
for ax in axs[len(mortgage.columns):]:
    ax.axis('off')
plt.tight_layout()
plt.show()'''
...
X = mortgage.drop(columns=['default_time']) #zmienne objaśniające
y = mortgage['default_time'] #zmienna objaśniana
...
#Metodu uczenia maszynowego - bez modyfikacji
...
train_data, test_data = train_test_split(mortgage, test_size=0.2)

#Stworzenie tablic typu np.array
train_y = np.array(train_data.pop('default_time'))
test_y = np.array(test_data.pop('default_time'))
